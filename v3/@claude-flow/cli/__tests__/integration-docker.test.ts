...
describe('Docker Compose Configuration', () => {
  ...
  it('mongodb exposes port 27017', () => {
    // ADR-166 §6 Phase 2b: bound to loopback only, not published to all interfaces
  ...
  it('chat-ui injects DOTENV_LOCAL with required env vars', () => {
    ...
    // ADR-166 §6 Phase 2b: MongoDB auth is on by default, so MONGODB_URL now
    // carries credentials (root user/pass) ahead of the mongodb:27017 host.
    expect(composeContent).toMatch(/MONGODB_URL=mongodb:\/\/.*@mongodb:27017\//);
...
describe('Cross-Service Port Consistency', () => {
  ...
  it('MCP bridge Dockerfile EXPOSE matches compose published port', () => {
    ...
    // ADR-166 §6 Phase 2b: published port is loopback-bound
